...
interface CheckoutPageProps {
  ...
  onLogoClick: () => void;
  onMenuClick: () => void;
  onProceedToPayment: (customerInfo: CustomerInfo, profileInfo?: ProfileInfo) => void;
}
...
export default function CheckoutPage({ cartCount, customerInfo, onBack, onLogoClick, onMenuClick, onProceedToPayment }: CheckoutPageProps) {
  const [formData, setFormData] = useState<CustomerInfo>(customerInfo);
  const [createProfile, setCreateProfile] = useState(false);
  const [profileData, setProfileData] = useState<ProfileInfo>({ email: '', password: '' });
  ...
  const handleProceedToPayment = () => {
    if (isFormValid) {
      onProceedToPayment(formData, createProfile ? profileData : undefined);
    }
  };
  ...
  return (
    <div className="bg-[#ffffff] relative size-full">
      ...
      {/* Form Fields */}
      <div className="absolute left-6 right-6 top-[286px] bottom-[140px] overflow-y-auto">
        <div className="box-border content-stretch flex flex-col gap-2 items-start justify-start p-0 pb-6 relative w-full">
          ...
          <div className="mt-4 relative shrink-0 w-full border-t border-neutral-200 pt-4">
            <div className="flex items-center justify-between w-full mb-3">
              <div className="flex flex-col">
                <span className="font-['Inter:Medium',_sans-serif] text-[16px] text-[#000000]">Create a profile</span>
                <span className="font-['Inter:Regular',_sans-serif] text-[12px] text-[#757575] mt-1">Save your info for faster checkout next time</span>
              </div>
              <Switch 
                checked={createProfile} 
                onCheckedChange={setCreateProfile}
                className="data-[state=checked]:bg-[#4C3F6C]"
              />
            </div>
            
            {/* Profile Fields - Only show when toggle is ON */}
            {createProfile && (
          ...
          <div className="relative shrink-0 w-full">
            <div className="box-border content-stretch flex flex-row font-normal items-center justify-start leading-[0] p-0 relative text-left w-full">
              <div className="basis-0 flex flex-col font-['Newsreader:Regular',_sans-serif] grow justify-center min-h-px min-w-px relative shrink-0 text-[#000000] text-[24px] tracking-[-0.48px]">
                <p className="block leading-[32px] text-[24px] text-[rgb(255,255,255)]">Shipping</p>
              </div>
            </div>
          </div>
        </div>
